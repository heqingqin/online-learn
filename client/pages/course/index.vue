        ...
        <section class="c-s-dl">
          <dl>
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                <li>
                  <a title="全部" href="#" @click="searchall()" :class="{active:allIndex==1}">全部</a>
                </li>
            ...
            <dd class="c-s-dl-li">
              <ul class="clearfix">
                <li v-for="(item,index) in subSubjectList" :key="index" :class="{active:twoIndex==index}">
                  <a :title="item.title" @click="searchTwo(item.id,index)" href="#">{{ item.title }}</a>
                </li>
              </ul>
            </dd>
          </dl>
...
export default {
 data(){
  ...
  methods:{
   ...
   //第一次查询课程
    initCourseFirst(){
      course.getCoursePage(1,8,this.searchObj)
      .then(response=>{
    ...
    //点击二级分类
    searchTwo(subjectParentId,index){
      // 让样式生效
      this.allIndex=-1
      this.twoIndex=index
    ...
    //切换分页的方法
      gotoPage(page) {
          course.getCoursePage(page, 8, this.searchObj)
            .then(response => {
              this.data = response.data.data
            })
