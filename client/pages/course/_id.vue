        ...
        <aside class="c-attr-wrap">
          <section class="ml20 mr15">
            ...
            <section v-else class="c-attr-mt">
              <a  title="立即观看" :href="'/player/'+firstvideoId" class="comm-btn c-btn-3">立即观看</a>
            </section>
          </section>
        </aside>
                          ...
                          <li class="lh-menu-stair" v-for="chapter in chapterVideoList" :key="chapter.id">
                            ...
                            <ol class="lh-menu-ol" style="display: block;">
                              <li class="lh-menu-second ml30" v-for="video in chapter.children" :key="video.id">
                                <a :href="'/player/'+video.videoSourceId" :title="video.title">
                                  <span class="fr">
...
import comment from '@/api/commoned'
export default {
  //异步调用，只调用一次
  ...
  methods:{
    //查询课程详细信息
    initCourseInfo(){
      course.getCourseFrontInfo(this.courseId)
        .then(response=>{
            this.chapterVideoList=response.data.data.chapterVideoList,
            this.firstvideoId=this.chapterVideoList[0].children[0].videoSourceId
    ...
    //分页跳转
    gotoPage(page){
      comment.getPageList(page, this.limit,this.courseId)
        .then(response => {
        this.data = response.data.data
      })
    }
